...
import (
	...
	clusterv1beta1 "github.com/michielvha/upjet-provider-confluent/apis/cluster/v1beta1"
	namespacedv1beta1 "github.com/michielvha/upjet-provider-confluent/apis/namespaced/v1beta1"
)
...
func TerraformSetupBuilder(version, providerSource, providerVersion string) terraform.SetupFn {
	return func(ctx context.Context, client client.Client, mg resource.Managed) (terraform.Setup, error) {
		...
		// Set credentials in Terraform provider configuration for Confluent Cloud
		ps.Configuration = map[string]any{}
		
		// Cloud API credentials (for organization/environment level resources)
		if v, ok := creds[keyCloudAPIKey]; ok {
			ps.Configuration[keyCloudAPIKey] = v
		}
		if v, ok := creds[keyCloudAPISecret]; ok {
			ps.Configuration[keyCloudAPISecret] = v
		}
		
		// Kafka cluster credentials (for data plane resources like topics, ACLs)
		if v, ok := creds[keyKafkaAPIKey]; ok {
			ps.Configuration[keyKafkaAPIKey] = v
		}
		if v, ok := creds[keyKafkaAPISecret]; ok {
			ps.Configuration[keyKafkaAPISecret] = v
		}
		
		// Schema Registry credentials
		if v, ok := creds[keySchemaRegistryAPIKey]; ok {
			ps.Configuration[keySchemaRegistryAPIKey] = v
		}
		if v, ok := creds[keySchemaRegistryAPISecret]; ok {
			ps.Configuration[keySchemaRegistryAPISecret] = v
		}
		
		// Endpoints
		if v, ok := creds[keyKafkaRestEndpoint]; ok {
			ps.Configuration[keyKafkaRestEndpoint] = v
		}
		if v, ok := creds[keySchemaRegistryURL]; ok {
			ps.Configuration[keySchemaRegistryURL] = v
		}
